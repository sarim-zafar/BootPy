...
def saving_project(Final,c,foot,x,nav,Name,title,f,proj_name):
    ...
    try:
        Proj_save=open(proj_name+".txt","w")
    except IOError:
        print("Error")
        
    if transparent_fixed==False:
        write.append("0\n<!--Section Ending>\n")
    else:
        write.append("1\n<!--Section Ending>\n")
    if scroll_nav==False:
...
def load_project(proj_name):
    ...
    Final=[]
    if footer_exists==True and nav_exists==True:
        for a in range((14+(num_pages)),len(load)-4):
            Final.append(load[a])
            
    elif ((footer_exists==True and nav_exists==False) or (footer_exists==False and nav_exists==True)):
        for a in range((14+(num_pages)),len(load)-3):
            Final.append(load[a])
            
    elif(footer_exists==False and nav_exists==False):
    ...
    elif footer_exists==False and nav_exists==True:
        nav=load[len(load)-2]
    elif footer_exists==True and nav_exists==False:
        foot=load[len(load)-2]
...
#A few arrays which handle the text spitted out by various modules and i plan on using techniques so that it can arrange what text to place where intelegently
...
while True:
    ...
    elif a=="9" or a=="load the project":
        proj_name=raw_input("Enter the name of project along without its extension(i.e proj):")
